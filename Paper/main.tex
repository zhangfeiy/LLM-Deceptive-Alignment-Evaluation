...
\begin{abstract}
Many recent papers use the word sycophancy to describe failures in preference aligned LLMs, but they do not always measure the same behavior. Some studies count whether a model agrees with a user's stated belief. Others test whether the model accepts a false premise, optimizes a reward proxy, or receives high scores from an LLM judge. These are related problems, but their evidence is not interchangeable. This review examines 48 studies on sycophancy, preference optimization, reward hacking, truthfulness evaluation, and deceptive alignment. The papers are coded by behavior type, prompt design, evaluation format, and metric type. The main finding is that sycophancy evaluation often suffers from prompt and metric mismatch. Agreement rate is useful for direct false agreement, but it is weak for hedged or partial correction. Truthfulness accuracy can miss whether the model actually corrected the user's false premise. Judge preference can reward polished and agreeable language instead of factual pushback. The review argues that future work should state the exact failure being tested and choose metrics that match that failure, rather than treating all sycophancy related results as directly comparable.
\end{abstract}
...
I treat this mismatch as the main object of the review, because it determines whether a reported sycophancy score is actually measuring the claimed failure. The question is not simply whether RLHF is worse than DPO, or whether one benchmark score is higher than another. The more basic question is whether the evaluation is measuring the failure it claims to measure. A paper using user opinion prompts and agreement rate is studying a different behavior from a paper using reward overoptimization curves or TruthfulQA style falsehood prompts. Treating all of these results as interchangeable evidence about sycophancy makes the literature look more settled than it is.

For that reason, the 48 papers are compared by the concrete choices that shape their results: what pressure the prompt creates, what behavior is counted as failure, and what metric is used to score that behavior. This framing exposes a practical weakness in current sycophancy evaluation: the field is good at counting obvious false agreement, but much less reliable when the model gives mixed, hedged, or socially polished responses that avoid direct correction.
...
The 48 studies were coded with overlapping labels because many papers do more than one thing. For example, one study may combine free form generation with LLM as a judge scoring, while another may report both benchmark accuracy and manual error analysis. I therefore treat the tables below as a map of evaluation choices, not as a set of mutually exclusive paper groups.
...
This is why the review treats prompt design, failure definition, and metric choice as inseparable. If two papers use the same word but different prompts and metrics, their results should not be placed on the same scale. The danger is using a narrow metric as if it covered the whole interaction.
